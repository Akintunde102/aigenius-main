  ...
  while (stack.length > 0 && out.length < maxFiles) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (!SKIP_DIR_NAMES.has(entry.name)) stack.push(abs);
...
function listSearchFiles(root: string, glob: string | undefined, maxFiles: number): string[] {
  ...
  try {
    ...
    if (stat.isFile()) {
      ...
      files = walkFiles(resolved, maxFiles);
    }
  } catch {
    return [];
  }
...
export function formatBundledGrepLines(
  result: BundledGrepResult,
  outputMode: BundledGrepOutputMode,
): string {
  if (!result.lines.length) return 'No matches found.';
  return result.lines.map((hit) => {
