...
def safe_print(text: str):
    """Prints text safely even on terminals with limited encoding support."""
    try:
        print(text)
    except UnicodeEncodeError:
        # Fallback for Windows CP1252: strip or replace non-ASCII characters
...
def print_success(text: str):
    safe_print(f"{Colors.GREEN}[PASSED] {text}{Colors.ENDC}")

def print_warning(text: str):
    safe_print(f"{Colors.YELLOW}[WARNING] {text}{Colors.ENDC}")

def print_error(text: str):
    safe_print(f"{Colors.RED}[FAILED] {text}{Colors.ENDC}")
