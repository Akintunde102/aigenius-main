...
// Remove Object since it's not supported by the API
...
// Add search limitation types to prevent array field search attempts
...
export type CallCommandsForSearch<T extends CObject, P> = {
  params: {
    searchableFields: SearchableField<T>[]; // Only non-array fields
    searchText: string
  };
...
export type Options<T extends CObject> = {
  ...
  populate?: {
    fields: {
      ...
      localKey: string;
      newField: string;
      multi?: boolean; // Add multi field for proper population functionality
    };
    space: string;
  }[];
};
